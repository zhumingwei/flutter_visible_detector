...
/// frames.
class VisibilityDetector extends SingleChildRenderObjectWidget with RouteAware2{
  /// Constructor.
  ///
  ...
  final ValueNotifier<RouterData> routeNotifier = ValueNotifier(RouterData(RouteType.didPush,null));
  ...
  void didPushNext(Route route) {
    ...
    routeNotifier.value = RouterData(RouteType.didPushNext, route);
  }

  @override
  void didPop() {
    super.didPop();
    routeNotifier.value = RouterData(RouteType.didPop, null);
  }

  @override
  void didPopNext(Route route) {
    super.didPopNext(route);
    routeNotifier.value = RouterData(RouteType.didPopNext, route);
  }

  @override
  void didPush() {
    super.didPush();
    routeNotifier.value = RouterData(RouteType.didPush, null);
  }
}
...
class RouterData {
  ...
  RouterData(this.routeType, this.extra);

  @override
  String toString() {
    return 'RouterData{routeType: $routeType, extra: $extra}';
  }
}
...
class VisibilityInfo {
  ...
  const VisibilityInfo({@required this.key, Size size, Rect visibleBounds , this.reason})
      : assert(key != null),
        size = size ?? Size.zero,
  ...
  @override
  String toString() {
    return '${visibleFraction} VisibilityInfo{key: $key, size: $size, visibleBounds: $visibleBounds reason: $reason';
  }
}
