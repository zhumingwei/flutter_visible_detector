...
class VisibilityDetectorLayer extends ContainerLayer {
  ...
  final ValueNotifier<RouterData> routeNotifier;
  ...
  dynamic get reason {
    if(VisibilityDetectorController.instance.currentState == AppLifecycleState.paused){
      return AppLifecycleState.paused;
    }
    if(routeNotifier.value.routeType == RouteType.didPop || routeNotifier.value.routeType == RouteType.didPushNext){
     return routeNotifier.value.routeType;
   }
   return null;
  }
  ...
  static void _processCallbacks() {
    for (final layer in _updated.values) {
      if (!layer.attached
          || VisibilityDetectorController.instance.currentState == AppLifecycleState.paused
          || (layer.routeNotifier!=null && layer.routeNotifier.value != null && (layer.routeNotifier.value.routeType == RouteType.didPushNext
          || layer.routeNotifier.value.routeType == RouteType.didPop))
      ) {
        layer._fireCallback(VisibilityInfo(
            key: layer.key, size: _lastVisibility[layer.key]?.size,reason: layer.reason));
        continue;
      }
  ...
  void attach(Object owner) {
    super.attach(owner);
    debugPrint("xxxxxx attach");
    _scheduleUpdate();
    VisibilityDetectorController.instance.stateNotifier.addListener(listenerToLifeCircleChanged);
  }
  ...
  void detach() {
    super.detach();
    debugPrint("xxxxxx detach");
    // The Layer might no longer be visible.  We'll figure out whether it gets
    // re-attached later.
    _scheduleUpdate();

    VisibilityDetectorController.instance.stateNotifier.removeListener(listenerToLifeCircleChanged);
  }

  void listenerToLifeCircleChanged(){
    _scheduleUpdate();
  }
